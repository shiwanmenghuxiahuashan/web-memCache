...
class MemCache {
  constructor(confing) {
    ...
    this.config = _.merge(defaultConfig, confing);
    this.logger = new Logger(this.confing);
  }
  ...
  parseCacheData(type, cacheData, options = {}) {
    ...
    return {
      ...cacheData,
      cacheValue: this._copy(cacheData.cacheValue),
    };
  }
  ...
  update(type, options, data) {
    ...
    if (!cacheData) {
      this.logger.error(` 更新 ${type} 缓存失败! 不存在该缓存，请使用 memCache.set(type,options,data) :`, cacheData);
      return false;
    }
   ...
   * @returns {boolean}
   */
  delete(type, options = { deleteRelatedResource: true }) {
    if (!type) return true;
